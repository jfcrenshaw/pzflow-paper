...
\begin{abstract}
    ...
    Emphasize that while this paper applies the technique to photo-z's, this technique is powerful for any galaxy properties or tabular data in general.
    \end{abstract}
...
PZFlow makes this easy to do automatically, by simply swapping \texttt{FlowEnsemble} for \texttt{Flow}.
...
\begin{figure*}[t!]
    ...
    \begin{centering}
        ...
        \caption{
            ...
            Note the posteriors for most galaxies are not this broad or multimodal. 
        }
...
\note{Think about what I want to do here.}
For example, we randomly select \note{N} galaxies and estimate redshift posteriors while marginalizing over the $u$ band.
These posteriors can be seen in Figure \note{N}.
Mention how the loss of information degrades the posteriors.
...
\begin{figure}[t!]
    ...
    \begin{centering}
        ...
        \caption{
            ...
            The black diagonal line represents perfect photo-z performance.
        }
...
\begin{figure*}[t!]
    ...
    \begin{centering}
        ...
        \caption{
            ...
            The dashed black lines represent the science requirements for LSST cosmology as stated in the LSST SRD \needscite.
...
This can be explained by the fact that normalizing flows exhibit mode covering behavior (the opposite of the mode collapse seen in GANsl; \citealt{salimans2016}).
In other words, because normalizing flows are trained via maximum likelihood estimation on the training samples, they receive very high penalties for missing any modes in the data.
As a result, they tend to conservatively spread out their density, in order to avoid any missed modes.
...
One of the purposes of this paper is to create a galaxy catalog for which each galaxy has a true redshift posterior, which will enable more comprehensive evaluation of photo-z estimators.  

\note{NEED TO FINISH WRITING THIS}
We will not undertake a full analysis of the posteriors here, but will compute a few metrics to point in that direction.
Calculate the KLD and maybe something else?
Point towards RAIL.
...
\begin{figure*}[t!]
    ...
    \begin{centering}
        ...
        \caption{
            ...
            After epoch 50 and 100, you can see a drop in the loss due to the decrease in the learning rate.
        }
...
\note{
    Need to add the curves for the ensembles.
    Move discussion of epochs and learning rates here too?    
}

\section{LSST Error Model}
\label{app:error-model}

We estimate photometric errors for LSST using the error model from \citet{ivezic2019}, except we do not make the high SNR approximation used there.

\citet{ivezic2019} calculates a Gaussian photometric error in magnitude space using Equation 5 from that paper: 
\begin{align}
    \sigma_\text{rand}^2 = (0.04 - \gamma) x + \gamma x^2 ~~ (\text{mag}^2),
\end{align}
where $x \equiv 10 ^ {0.4(m - m_5)}$, $m_5$ is the 5$\sigma$ depth for the band, and $\gamma$ is a band-dependent parameter listed in Table 2 of the same paper.
This formula is based on the approximation
\begin{align}
    \sigma_\text{rand} = 2.5 \log_{10} \left(1 + \frac{N}{S} \right) \approx \frac{N}{S},
\end{align}
where $S$ and $N$ are the flux signal and noise, respectively.
This approximation is valid in the high signal-to-noise regime.

To generalize this model to the low signal-to-noise regime, we abandon this latter approximation.
For each magnitude, $m$, we calculate the corresponding flux, $S = 10^{-m/2.5}$, and the Gaussian flux noise,
\begin{align}
    N = S \, \sqrt{(0.04 - \gamma) x + \gamma x^2}, \label{eq:noise}.
\end{align}
For very bright galaxies, we impose a systematic floor corresponding to an error of 0.005 mags.
We then sample an observed flux, $S_\text{obs} \sim \mathcal{N}(S, N)$, and calculate the observed magnitude, $m_\text{obs} = -2.5 \log_{10} S_\text{obs}$.

For the reported photometric error, we do not use $N$, as this provides a deterministic link to the true magnitude via Equation \ref{eq:noise}.
Instead, we apply Equation \ref{eq:noise} again, this time using the observed magnitude instead of the true magnitude, yielding the estimated noise, $N_\text{obs}$.
Finally, we report the photometric error in magnitudes:
\begin{align}
    \sigma_m = 2.5 \log_{10} \left(1 + \frac{N_\text{obs}}{S_\text{obs}} \right).
\end{align}
