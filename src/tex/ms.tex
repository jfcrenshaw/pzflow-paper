...
We sample a size and ellipticity for each galaxy in the PZFlow catalog created in the previous section (using the true magnitudes), and plot the distribution of these features against the distribution in the test set (Figure \ref{fig:conditional-corner}).
...
This small catalog was generated for visualization purposes, but the normalizing flows can be used to generate catalogs of arbitrarly large size (see Appendix \ref{app:benchmarking} on benchmarking PZFlow).
...
This makes them useful tools for estimating posterior distributions for galaxy properties, conditioned on observed properties of the galaxy.
...
In the machine learning literature, this is known as a Deep Ensemble \needscite, a popular method for approximate bayesian marginalization \needscite.
...
To account for the photometric error, we resample the training set from the error distributions on each epoch.
...
Each flow estimates the posterior by marginalizing over the error distribution.
We then average the posteriors from each flow in the ensemble.
An example of this can be seen in Figure \ref{fig:ensemble-posteriors}.
...
This might be useful when not every galaxy is observed in every band, in which case you can marginalize over the bands that were not observed, or in cases when a galaxy was observed but not detected in the band.
You can then marginalize over values for the band that are beyond the limiting magnitude.
This can also help you understand what information is included in each band and how the inclusion of that band improves photo-z estimation.
...
At some point in this section, note that we estimate posteriors for different galaxies than we trained on.
...
The most common point of comparison is photo-z point estimates.
We make the common choice of selecting the mode of the posteriors.
...
PIT metric.
\note{something something.}
Discuss the mode covering behavior of normalizing flows and how this relates to the PIT diagram.
...
Binned point metrics.
Figure \ref{fig:binned-metrics}
Defined in the science book.
We plot against the requirements from the LSST SRD just to provide a reference for the scale.
Seems to do well up to z~1.5.
We note that for cosmology, it is okay for the bias to exceed the required limits, as long as the bias can be well determined via some calibration process, e.g. clustering redshifts.

The previous metrics concerned photo-z point estimates, which are insufficient for modern science.
The DC1 paper demonstrated that these metrics are not sufficient to validate the full posteriors produced by photo-z codes.
The purpose of this paper is to produce catalogs with true posteriors, which we have done, so that you can evaluate metrics of the full posterior.
