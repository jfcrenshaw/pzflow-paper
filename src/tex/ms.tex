...
\begin{abstract}
    Normalizing flows are powerful tools for learning high-dimensional probability distributions from samples thereof.
    We introduce PZFlow, a Python package for the statistical modeling of tabular data using normalizing flows.
    We use PZFlow to model the seven dimensional space of galaxy redshift and LSST photometry.
    Using these normalizing flows, we forward model galaxy catalogs in which each galaxy property has an associated ``true'' posterior distribution.
    We discuss the importance of these posteriors for the comprehensive evaluation of the redshift posteriors produced by photometric redshift (photo-z) estimators.
    We also demonstrate the use of an ensemble of normalizing flows for photo-z estimation.
    While we focus on photo-z estimation and validation, we emphasize that these methods are applicable to any galaxy properties, or any other tabular data.
\end{abstract}
